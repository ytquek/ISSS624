...
```{r}
hunan_ehsa <- emerging_hotspot_analysis(
  x = GDPPC_st,  .var = "GDPPC",
  k = 1,  nsim = 99
)
```

```{r}
ehsa_sig <- hunan_ehsa %>%  filter(p_value <0.05) tmap_mode("plot")tm_shape(hunan_ehsa) +
  tm_polygons() +  tm_borders(alpha = 0.5) +
tm_shape(ehsa_sig) +  tm_fill("classification") +
